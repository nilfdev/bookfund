...
describe("Token Transfers", function () {
  ...
  it("Should be possible for the owner transfer tokens to another account", async function () {
    ...
    assert.equal(lucasBalance, 0, "Lucas has not an empty balance");
    ...
    assert.equal(ownerBalance, 4990000, "Owner balance was not updated correctly");
    assert.equal(lucasBalance, 10000,"Lucas balance was not updated correctly");
  });
  ...
  it("Should not be possible for an account to trasnfer more than it owns", async function () {
    ...
    assert.equal(lucasBalance, 10000, "Lucas has not the correct balance");
    assert.equal(joaoBalance, 0, "Joao has not an empty balance");

    await expect(token.connect(lucas).transfer(await joao.getAddress(), 15000)).to.be.reverted;

    lucasBalance = await token.balanceOf(await lucas.getAddress());
    joaoBalance = await token.balanceOf(await joao.getAddress());

    assert.equal(lucasBalance, 10000, "Lucas balance was not supposed to change");
    assert.equal(joaoBalance, 0, "Joao balance was not supposed to change");
  });
  ...
  it("Should be possible for an account to transfer less than it owns", async function () {
    ...
    assert.equal(lucasBalance, 10000, "Lucas has not the correct balance");
    assert.equal(carolBalance, 0, "Carol has not an empty balance");
    ...
    assert.equal(lucasBalance, 5000, "Lucas balance was not updated correctly");
    assert.equal(carolBalance, 5000, "Carol balance was not updated correctly");
  });
});
